...
export const usePenugasan = () => {
  ...
  const loadPenugasanData = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
  ...
  const handleEdit = useCallback(async (id, wilayahList, usersList) => {
    try {
      setLoading(true);
      const response = await penugasanAPI.getById(id);
      ...
      return {
        ...
        selectedUsers,
        selectedWilayah,
        wilayahCheckboxState,
        individuCheckboxState,
        selectAllWilayah: selectedWilayah.length === wilayahList.length && wilayahList.length > 0,
        selectAllIndividu: selectedUsers.length === usersList.filter(u => u.roles === 'pegawai' && u.is_active).length
      };
    } catch (error) {
  ...
  const handleDelete = useCallback(async (id, nama_penugasan, tipe_penugasan, status, is_active, onSuccess) => {
  ...
  const handleSoftDelete = useCallback(async (id, nama_penugasan, onSuccess) => {
  ...
  const handleUpdateStatus = useCallback(async (id, status, nama_penugasan, onSuccess) => {
  ...
  const handleViewMonitoring = useCallback(async (penugasan, tanggal, setMonitoringData, setShowMonitoringModal, setSelectedPenugasan) => {
    try {
      setLoading(true);
